...
function addPerson(username, password, extraData = {display_name_en: 'DNE'}, isTest = true, ignoreDuplicate = false) {
  return new Promise((resolve, reject) => {
    ...
    if (!extraData.display_name_en)
      extraData.display_name_en= 'admin';
...
function loginPerson(username, password) {
  return new Promise((resolve, reject) => {
    ...
    rp({
      ...
      .then(() => resolve(rpJar))
      .catch(err => reject(`could not login '${username}' with '${password}':\n  ${helpers.parseServerErrorToString(err)}`))
  })
}
...
function connectBizWithRep(pid, bid, isActive = true) {
  return sql.test.association.add({pid: pid, bid: bid})
    .then(res => {
