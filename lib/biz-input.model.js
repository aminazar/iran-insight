...
class BizInput extends SqlTable {
  constructor(tableName, columns, test, idColumn, formatter = d => d, cyclic = defaultCyclic) {
      ...
      let saver = () => {
        this.sql.association.add({bid, pid})
          .then(res => {
            ...
            return this.cyclic(this.sqlTable, bid);
          })
          .then(res => {
            if(res)
              data.investment_cycle = res;
  ...
  get(id) {
    return this.sqlTable.getDetails({id: id})
      .then(data => Promise.resolve(this.formatter(data)[0]));
  }
}
