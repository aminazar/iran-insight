...
class BizInput extends SqlTable {
  ...
  getByBiz(bid, getAll = false) {
    return new Promise((resolve, reject) => {
      this.sqlTable.getByBiz({bid: bid, is_confirmed: getAll ? null : true})
        .then(data => resolve(this.formatter(data)))
        .catch(reject);
    });
  }

  getByOrg(oid, getAll = false) {
    return new Promise((resolve, reject) => {
      this.sqlTable.getByOrg({oid: oid, is_confirmed: getAll ? null : true})
        .then(data => resolve(this.formatter(data)))
        .catch(reject);
    });
  }

  getByPerson(pid, getAll = false) {
    return new Promise((resolve, reject) => {
      this.sqlTable.getByPerson({pid: pid, is_confirmed: getAll ? null : true})
        .then(data => resolve(this.formatter(data)))
        .catch(reject);
    });
  }
  ...
  confirm(id, user) {
    return new Promise((resolve, reject) => {
      ...
      this.sqlTable.getWithAssoc({id})
        ...
        .then(res => {
          if(!res.length)
            return Promise.reject(Err.notConfirmer);
  ...
  get(id, user_id) {
    return new Promise((resolve, reject) => {
      this.db.task(t => {
        return this.sql.person.isAdmin({pid: user_id}, t)
          .then(res => {
            if(res.length > 0)
              return Promise.resolve(res);
            else
              return this.sqlTable.getBizIdById({id: id}, t)
                .then(res => {
                  return this.sql.person.bizRep({pid: user_id, bid: res[0].bid}, t);
                })
                .then(res => {
                  if(res.length > 0)
                    return Promise.resolve(res);

                  return this.sqlTable.hasAccess({user_id: user_id, id: id});
                });
          })
          .then(res => {
            ...
            return this.sqlTable.getDetails({id: id, is_confirmed: isConfirmed}, t);
          })
          .then(data => resolve(this.formatter(data)[0]))
          .catch(err => {
