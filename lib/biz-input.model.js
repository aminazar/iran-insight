          ...
          .then(res => {
            this.sqlTable.getDetails({id: res.id})
              .then(result => {
                let msg = {
                  from: user,
                  about: (this.tableName === 'investment') ? Notification.get().getNotificationCategory().PersonClaimInvestmentOnBusiness : Notification.get().getNotificationCategory().PersonClaimConsultingBusiness,
                  aboutData: {
                ...
                Notification.get().pushNotification(msg, {bid: bid});
              });
          ...
          .then(res => {
            this.sqlTable.getDetails({id: res.id})
              .then(result => {
                let msg = {
                  from: user,
                  about: (this.tableName === 'investment') ? Notification.get().getNotificationCategory().OrganizationClaimInvestmentOnBusiness : Notification.get().getNotificationCategory().OrganizationClaimConsultingBusiness,
                  aboutData: {
                    person_name: (user.firstname_en || user.firstname_fa) + ' ' + (user.surname_en || user.surname_fa),
                    person_username: user.username,
                    business_name: result[0].business_name || result[0].business_name_fa,
        ...
        .then(res => {
          this.sqlTable.getDetails({id: id})
            .then(result => {
              let msg = {
                from: user,
                about: (this.tableName === 'investment') ? Notification.get().getNotificationCategory().AcceptInvestment : Notification.get().getNotificationCategory().AcceptConsulting,
                aboutData: {
                  business_name: result[0].business_name || result[0].business_name_fa,
                }
              };

              (result[0].person_id) ? this.sql.person.getUserById({pid: result[0].person_id}) : this.sql.organization.getById({oid: result[0].organization_id})
                .then(xData => {
                  let msg_broadcast = {
                    from: user,
                    aboutData: {
                      person_name: result[0].person_id ? (xData.firstname_en || xData.firstname_fa) + ' ' + (xData.surname_en || xData.surname_fa) : null,
                  ...
                  if(this.tableName === 'investment' && result[0].person_id)
                    msg_broadcast = Object.assign({about: Notification.get().getNotificationCategory().PersonInvestsOnBusiness}, msg_broadcast);
                  else if(this.tableName === 'investment' && result[0].organization_id)
                    msg_broadcast = Object.assign({about: Notification.get().getNotificationCategory().OrganizationInvestsOnBusiness}, msg_broadcast);
                  else if(this.tableName === 'consultancy' && result[0].person_id)
                    msg_broadcast = Object.assign({about: Notification.get().getNotificationCategory().PersonConsultingBusiness}, msg_broadcast);
                  else if(this.tableName === 'consultancy' && result[0].organization_id)
                    msg_broadcast = Object.assign({about: Notification.get().getNotificationCategory().OrganizationConsultingOnBusiness}, msg_broadcast);

                  Notification.get().pushNotification(msg_broadcast);
                });

              Notification.get().pushNotification(msg, result[0].person_id ? {pid: result[0].person_id} : {oid: result[0].organization_id});
            });
          ...
          this.sqlTable.getDetails({id: data.bid})
            .then(result => {
              ...
              if(isApprover && this.tableName === 'investment')
                msg = Object.assign({about: Notification.get().getNotificationCategory().RejectInvestmentOnBusiness}, msg);
              else if(isApprover && this.tableName === 'consultancy')
                msg = Object.assign({about: Notification.get().getNotificationCategory().RejectConsultancyOnBusiness}, msg);
              else if(!isApprover && this.tableName === 'investment')
                msg = Object.assign({about: Notification.get().getNotificationCategory().RemoveInvestmentOnBusiness}, msg);
              else if(!isApprover && this.tableName === 'consultancy')
                msg = Object.assign({about: Notification.get().getNotificationCategory().RemoveConsultancyOnBusiness}, msg);
