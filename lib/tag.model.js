...
class Tag extends SqlTable {
  ...
  confirm(tagId) {
    ...
    return this.sql.tag.update({proposer: `'{"business" :[] , "organization":[] , "product":[]}'::jsonb`}, tagId);
  }
  ...
  setTag(user_pid, body) {
    ...
    let exec = (type) => {
      ...
      let checkAccess;
      if (type === types.BIZ)
        checkAccess = this.getUserAccess(user_pid, body.bid, null);
      else if (type === types.ORG)
        checkAccess = this.getUserAccess(user_pid, null, body.oid);
      else if (type === types.PROD)
        checkAccess = new Promise((resolve, reject) => {
