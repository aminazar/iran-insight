    ...
    return new Promise((resolve, reject) => {
      this.sql[this.tableName].get(criteria)
        .then(data => {
          ...
          if (data.length > 1) {
            let err = new Error(`Not unique record with criteria: ${JSON.stringify(criteria)} in table ${this.tableName}`);
            err.status = 400;
            reject(err);
          } else if (!data.length) {
            let err = new Error(`No records with criteria: ${JSON.stringify(criteria)} in table ${this.tableName}`);
            err.status = 404;
            reject(err);
          } else {
