...
class SqlTable {
  ...
  clean(data) {
    let exprt = {};
    Object.keys(data).filter(k => !['sql','tableName','columns','test','idMember'].includes(k)).forEach(col => {
  ...
  getUserAccess(user_id, bid = null , oid = null,  task = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let func ;
      if (task)
        func= this.sql.membership.isRepresentativeOrAdmin({pid: user_id, bid: bid, oid: oid}, task);
        else
        func= this.sql.membership.isRepresentativeOrAdmin({pid: user_id, bid: bid, oid: oid});
