...
class Organization extends SqlTable {
            ...
            this.sql[tableName].getById({oid: data.oid})
              .then(res => {
                if (data.oid) {
                  let msg = {
                    from: res[0],
                    about: Notification.getNotificationCategory().OrganizationUpdateProfile,
                    aboutData: {
  ...
  pushingLCENotification(organization_id, lce_description, lce_description_fa, isConfirmed = false) {
    this.sql[tableName].get({oid: organization_id})
      .then(res => {
        let msg = {
          from: res[0],
          about: isConfirmed ? Notification.getNotificationCategory().OrganizationAddLifeCycleEvent : Notification.getNotificationCategory().OrganizationRequestLifeCycleEvent,
          aboutData: {
            organization_name: res[0].name || res[0].name_fa,
            ...
            if (!body.id) { // Insert
                      ...
                      if (body.oid2) {
                        this.sql.person.getUserById({pid: user_id}).then(rep1 => { // send email to rep2 about lce request by rep1
                          this.sql.membership.getOrgRep({oid: body.oid2}).then(rep2 => {
                            ...
                            // });
                            this.pushingLCENotification(body.oid1, body.description, body.description_fa, body.is_confirmed);
                            resolve(lceId);
              ...
              this.sql.organization_lce.update(body, body.id).then(res => {
                this.sql.organization_lce.get({lce_id: body.id})
                  .then(res => this.pushingLCENotification(res[0].oid1, res[0].description, res[0].description_fa, res[0].is_confirmed));
                resolve(res);
              }).catch(err => reject(err));
            }
          }
            ...
            this.sql.organization_lce.update({is_confirmed: body.is_confirmed}, body.id).then(res => {
              this.sql.organization_lce.getOrganizationLCEData({id: body.id})
                .then(res => {
                  let msg_confirm = {
                    from: {oid: body.oid},
                    about: Notification.getNotificationCategory().OrganizationConfirmLifeCycleEvent,
                    aboutData: {
                  ...
                  let msg_broadcast = {
                    about: Notification.getNotificationCategory().TwoOrganizationAddLifeCycleEvent,
                    aboutData: {
                  ...
                  //ToDo: call notification method (for former)
                  //ToDo: Broadcast notification (former)
                  //ToDo: Broadcast notification (seconder)
                });
              resolve(res);
            }).catch(err => reject(err));
          else
            this.sql.organization_lce.getOrganizationLCEData({id: body.id})
              ...
              .then(res => {
                let msg = {
                  ...
                  about: Notification.getNotificationCategory().OrganizationRejectLifeCycleEvent,
                  aboutData: {
  ...
  deleteLCE(user, body) {
    return new Promise((resolve, reject) => {
      this.sql.organization_lce.getOrganizationLCEData({id: body.id}).then(lce => {
        this.getUserAccess(user.pid, lce[0].oid1).then(res => {
          if (res.isAdmin || res.isRep) { // is admin or rep of first org
            this.sql.organization_lce.delete(body.id).then(() => {
              let msg = {
                from: (res.isAdmin) ? user : {bid: lce[0].former_bid},
                about: Notification.getNotificationCategory().OrganizationRemoveLifeCycleEvent,
                aboutData: {
                  admin_name: (user.firstname_en || user.firstname_fa) + ' ' + (user.surname_en || user.surname_fa),
                  admin_username: user.username,
                  isAdmin: res.isAdmin,
                  organization_name: lce[0].former_oid,
                  lce_description: lce[0].description || lce[0].description_fa,
                }
              };

              //ToDo: call notification method (if isAdmin should notify both first and second orgs)
              resolve();
            }).catch(err => reject(err));
          } else {
            this.getUserAccess(user.pid, lce[0].oid2).then(res => {
                ...
                this.sql.organization_lce.delete(body.id).then(() => {
                  let msg = {
                    ...
                    aboutData: {
                      ...
                      admin_username: user.username,
                      isAdmin: res.isAdmin,
                      organization_name: lce[0].seconder_oid,
                      lce_description: lce[0].description || lce[0].description_fa,
                    }
                  };
