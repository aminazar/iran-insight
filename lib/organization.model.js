            ...
            this.sql[tableName].getById({oid: data.oid})
              .then(res => {
                if (data.oid) {
                  let msg = {
                    from: res[0],
                    about: Notification.get().getNotificationCategory().OrganizationUpdateProfile,
                    aboutData: {
  ...
  pushingLCENotification(organization_id, lce_description, lce_description_fa, isConfirmed = false, other_organization_id) {
    this.sql[tableName].get({oid: organization_id})
      .then(res => {
        let msg = {
          from: res[0],
          about: isConfirmed ? Notification.get().getNotificationCategory().OrganizationAddLifeCycleEvent : Notification.get().getNotificationCategory().OrganizationRequestLifeCycleEvent,
          aboutData: {
            organization_name: res[0].name || res[0].name_fa,
        ...
        if(isConfirmed)
          ...
          Notification.get().pushNotification(msg, {oid: other_organization_id});
      });
  }
            ...
            this.sql.organization_lce.update({is_confirmed: body.is_confirmed}, body.id).then(res => {
              this.sql.organization_lce.getOrganizationLCEData({id: body.id})
                .then(res => {
                  let msg_confirm = {
                    from: {oid: body.oid},
                    about: Notification.get().getNotificationCategory().OrganizationConfirmLifeCycleEvent,
                    aboutData: {
                  ...
                  let msg_broadcast = {
                    about: Notification.get().getNotificationCategory().TwoOrganizationAddLifeCycleEvent,
                    aboutData: {
                  ...
                  Notification.get().pushNotification(msg_confirm, {oid: res[0].former_oid});
            ...
            this.sql.organization_lce.getOrganizationLCEData({id: body.id})
              ...
              .then(res => {
                let msg = {
                  ...
                  about: Notification.get().getNotificationCategory().OrganizationRejectLifeCycleEvent,
                  aboutData: {
                ...
                Notification.get().pushNotification(msg, {oid: organizationLCEDetails.former_oid});
          ...
          if (res.isAdmin || res.isRep) { // is admin or rep of first org
            this.sql.organization_lce.delete(body.id).then(() => {
              let msg = {
                ...
                about: Notification.get().getNotificationCategory().OrganizationRemoveLifeCycleEvent,
                aboutData: {
              ...
              if(res.isAdmin){
                Notification.get().pushNotification(msg, {oid: lce[0].former_oid});
                if(lce[0].seconder_oid)
                  Notification.get().pushNotification(msg, {oid: lce[0].seconder_oid});
              }
              else{
                Notification.get().pushNotification(msg);
