      ...
      this.loadOrData(eid, data)
        ...
        .then(res => {
          ...
          if (oldData.organizer_pid)
            getOrganizer = this.sql.person.getUserById({pid: oldData.organizer_pid});
          ...
          getOrganizer
            .then(res => {
              let msg = {
                from: user,
                about: Notification.get().getNotificationCategory().OrganizerAddUpdateEvent,
                aboutData: {
                  organizer_name: (oldData.organizer_pid ? (res[0].firstname_en || res[0].firstname_fa) + ' ' + (res[0].surname_en || res[0].surname_fa) : (res[0].name || res[0].name_fa)),
        ...
        .then(res => {
          ...
          getOrganizer
            .then(res => {
              let msg = {
                from: user,
                about: Notification.get().getNotificationCategory().OrganizerRemoveEvent,
                aboutData: {
                  organizer_name: (oldData.organizer_pid ? (res[0].firstname_en || res[0].firstname_fa) + ' ' + (res[0].surname_en || res[0].surname_fa) : (res[0].name || res[0].name_fa)),
                  organizer_username: res[0].username,
                  is_updated: eid !== undefined || eid !== null,
