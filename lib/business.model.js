...
class Business extends SqlTable {
      ...
      this.sql[membershipTable].isRepresentativeOrAdmin({
        ...
        .then(res => {
          if (res.length === 0)
            ...
            data.start_date = (data.start_date ? moment(data.start_date) : moment()).format('YYYY-MM-DD');
  ...
  deleteBiz(bid, user_id, end_date){
    let mid = null;
    let aid = null;
    ...
    return this.db.task(t =>
      this.sql[membershipTable].isRepresentativeOrAdmin({
        ...
        .then(res => {
          ...
          return this.sql[tableName].delete({end_date: endDate, bid: bid}, t);
        })
        // .then(res => {
        //   if(!res || res.length <= 0)
        //     return Promise.reject(error.notBizRep);
        //
        //   return this.sql.membership.getAssocWithBizRep({bid: bid, pid: user_id}, t);
        // })
        // .then(res => {
        //   mid = (res && res.length > 0) ? res[0].mid : null;
        //   aid = (res && res.length > 0) ? res[0].aid : null;
        //   if(mid)
        //     return this.sql.membership.delete(mid, t);
        //   else
        //     return Promise.resolve();
        // })
        // .then(res => {
        //   if(aid)
        //     return this.sql.association.delete(aid, t);
        //   else
        //     return Promise.resolve();
        // })
        // .then(res => this.sql.business.delete(bid, t))
        .then(res => {
          return Promise.resolve('');
        })
        .catch(err => {
