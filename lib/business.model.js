        ...
        .then(res => {
          if (res.isAdmin || res.isRep) { // biz rep or admin is insert/update lce
            ...
            if (!body.id) { // add new expertise
              ...
              this.sql.business_lce.update(body, body.id).then(res => {
                resolve(res);
              }).catch(err => reject(err));
            }
          }
      ...
      this.getUserAccess(user_id, body.bid, null).then(res => {
        ...
        if (res.isAdmin || res.isRep) { // biz rep or admin is insert/update lce
          ...
          if (body.is_confirmed)
            ...
            this.sql.business_lce.delete(body.id).then(res => {
              resolve(res);
            }).catch(err => reject(err));
        } else {
          reject(error.notAllowed);
  ...
  deleteLCE(user_id, body) {
    return new Promise((resolve, reject) => {
      ...
      this.sql.business_lce.get({id: body.id}).then(lce => {
        this.getUserAccess(user_id, lce[0].bid1, null).then(res => {
          ...
          if (res.isAdmin || res.isRep) { // is admin or rep of first biz
            ...
            this.getUserAccess(user_id, lce[0].bid2, null).then(res => {
