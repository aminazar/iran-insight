...
class Business extends SqlTable {
  ...
  pushingLCENotification(business_id, lce_description, lce_description_fa, isConfirmed = false, other_business_id) {
    this.sql[tableName].get({bid: business_id})
      .then(res => {
        let msg = {
          from: res[0].bid,
          about: isConfirmed ? NotificationCategory.BusinessAddLifeCycleEvent : NotificationCategory.BusinessRequestLifeCycleEvent,
          aboutData: {
            business_name: res[0].name || res[0].name_fa,
        ...
        if (isConfirmed)
          ...
          NF.pushNotification(msg, {bid: other_business_id});
      });
  }
  ...
  setLCE(body, user_id) {
    return new Promise((resolve, reject) => {
      //Check user accessibility
      this.getUserAccess(user_id, body.bid1, null)
        .then(res => {
  ...
  confirmLCE(user_id, body) {
    return new Promise((resolve, reject) => {
      ...
      this.getUserAccess(user_id, body.bid , null).then(res => {
        if (res.isAdmin || res.isRep) { // biz rep or admin is insert/update lce
            ...
            this.sql.business_lce.update({is_confirmed: body.is_confirmed}, body.id).then(res => {
              this.sql.business_lce.getBusinessLCEData({id: body.id})
                .then(res => {
                  let msg_confirm = {
                    from: {bid: body.bid},
                    about: NotificationCategory.BusinessConfirmLifeCycleEvent,
                    aboutData: {
                  ...
                  NF.pushNotification(msg_confirm, {bid: res[0].former_bid});
            ...
            this.sql.business_lce.getBusinessLCEData({id: body.id})
              ...
              .then(res => {
                let msg = {
                  ...
                  about: NotificationCategory.BusinessRejectLifeCycleEvent,
                  aboutData: {
                ...
                NF.pushNotification(msg, {bid: businessLCEDetails.former_bid});
  ...
  deleteLCE(user, body) {
    return new Promise((resolve, reject) => {
      this.sql.business_lce.getBusinessLCEData({id: body.id}).then(lce => {
        this.getUserAccess(user.pid, lce[0].bid1 , null).then(res => {
          if (res.isAdmin || res.isRep) { // is admin or rep of first biz
            this.sql.business_lce.delete(body.id).then(() => {
              ...
              if (res.isAdmin) {
                NF.pushNotification(msg, {bid: lce[0].former_bid});
                if (lce[0].seconder_bid)
                  NF.pushNotification(msg, {bid: lce[0].seconder_bid});
              }
              else {
                NF.pushNotification(msg);
            ...
            this.getUserAccess(user.pid, lce[0].bid2, null).then(res => {
  ...
  getLCE(user_pid, param_bid) {
    return new Promise((resolve, reject) => {
      ...
      this.getUserAccess(user_pid, param_bid, null).then(res => {
...
//       Promise.reject(error.noBusinessIdDeclare)
//       :
//       this.sql[businessProductTable].getByProductId({product_id})
//         .then(res=> {
//           if(res.length === 0){
//             return this.sql[productTable].delete(product_id)
//               .then(res=>{
...
//           }
//           else {
//             return this.sql[businessProductTable].deleteBizProductByAdmin({product_id: product_id})
//           }
//         })
//         .then(res=>{
//           return this.sql[productTable].delete({id: product_id});
//         })
//         .then(res=>{
//           resolve(res);
//         })
