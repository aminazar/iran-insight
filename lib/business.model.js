...
class Business extends SqlTable {
  ...
  pushingLCENotification(business_id, lce_description, lce_description_fa, isConfirmed = false) {
    console.log('In the pushingLCENotification');
                      ...
                      if (body.bid2) {
                        this.sql.person.getUserById({pid: user_id}).then(rep1 => { // send email to rep2 about lce request by rep1
                          this.sql.membership.getBizRep({bid: body.bid2}).then(rep2 => {
                            ...
                            // });
                            this.pushingLCENotification(body.bid1, body.description, body.description_fa, body.is_confirmed);
                            resolve(lceId);
            ...
            this.sql.business_lce.update({is_confirmed: body.is_confirmed}, body.id).then(res => {
              this.sql.business_lce.getBusinessLCEData({id: body.id})
                .then(res => {
                  let msg_confirm = {
                    from: {bid: body.bid},
                    about: Notification.getNotificationCategory().BusinessConfirmLifeCycleEvent,
                    aboutData: {
                  ...
                  let msg_broadcast = {
                    about: Notification.getNotificationCategory().TwoBusinessAddLifeCycleEvent,
                    aboutData: {
            ...
            this.sql.business_lce.getBusinessLCEData({id: body.id})
              ...
              .then(res => {
                let msg = {
                  ...
                  about: Notification.getNotificationCategory().BusinessRejectLifeCycleEvent,
                  aboutData: {
          ...
          if (res.isAdmin || res.isRep) { // is admin or rep of first biz
            this.sql.business_lce.delete(body.id).then(() => {
              let msg = {
                ...
                about: Notification.getNotificationCategory().BusinessRemoveLifeCycleEvent,
                aboutData: {
                  admin_name: (user.firstname_en || user.firstname_fa) + ' ' + (user.surname_en || user.surname_fa),
                  admin_username: user.username,
                  isAdmin: res.isAdmin,
                  business_name: lce[0].former_bid,
                  lce_description: lce[0].description || lce[0].description_fa,
                }
              };

              //ToDo: call notification method (if isAdmin should notify both first and second biz)
              resolve();
            }).catch(err => reject(err));
          } else {
                ...
                this.sql.business_lce.delete(body.id).then(() => {
                  let msg = {
                    ...
                    aboutData: {
                      ...
                      admin_username: user.username,
                      isAdmin: res.isAdmin,
                      business_name: lce[0].seconder_bid,
                      lce_description: lce[0].description || lce[0].description_fa,
                    }
                  };
