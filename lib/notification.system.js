...
class NotificationSystem {
  ...
  sendMailToRecipient(type) {
    if (type !== notificationType.Weekly && type !== notificationType.Daily)
      return;
    ...
    this.sql.person.get({notify_period: type}).then(res => {
      ...
      if (res.length > 0) {
        ...
        res.forEach(person => {
          this.fetchNotifications(person.pid).forEach(note => {

            let message = this.composeMail(type, note, person.pid, Person.getUserUnsubscribeHash(person));
            helpers.sendMail(message.body_plain, message.body_html, message.subject, person.username)
              .then(() => this.deleteNotification(person.pid))
              .catch(err => console.log('-> ', 'failed to send email notification'));
          });
        });
      }
    });
