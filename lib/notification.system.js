...
class NotificationSystem {
  ...
  composeMail(messageType, data, user_id, user_subscription_hash) {
    ...
    if (data.length === 1) {
      ...
      message.body_plain += 'You have new message:\n' +
      ...
      actionRequiredObjects.forEach(el => {
        ...
        actionRequiredMessages_plain += '\t' + messageContentObject.msg + '\nClick link to see message: ' + messageContentObject.link;
      });
      ...
      businessObjects.forEach(el => {
        ...
        businessMessages_plain += '\t' + messageContentObject.msg + '\nClick to see message: ' + messageContentObject.link;
      });
      ...
      organizationObjects.forEach(el => {
        ...
        organizationMessages_plain += '\t' + messageContentObject.msg + '\nClick to see message: ' + messageContentObject.link;
      });
  ...
  composeMessage(category, data) {
    switch (category) {
      ...
      case notificationCategory.UserUpdateProfile:
        return {
          msg: (data.person_name !== ' ' ? data.person_name : data.person_username) + ' was updated his/her profile data',
          link: baseLink + '/user/profile/' + data.person_id,
        };
        break;
      case notificationCategory.AcceptRepRequest:
        return {
          msg: (data.person_name !== ' ' ? data.person_name : data.person_username) + ' accepts your request to being representative of ' + data.org_biz_name + (data.is_business ?  ' business' : ' organization'),
          link: baseLink + '/notifications',
        };
        break;
      case notificationCategory.RejectRepRequest:
        return {
          msg: (data.person_name !== ' ' ? data.person_name : data.person_username) + ' rejects your request to being representative ' + data.org_biz_name + (data.is_business ?  ' business' : ' organization'),
          link: baseLink + '/notifications',
        };
        break;
