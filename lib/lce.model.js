  ...
  getLCE(type, user_pid, param_id) {
    ...
    return exec.then(res => {
      if (res.isAdmin || res.isRep) { // rep or admin is getting lce
        ...
        return this.sql[`${type}_lce`].getLCEList({joinerId: param_id, condition: 'true = true'}); // show all LCE
      } else {
        return this.sql[`${type}_lce`].getLCEList({joinerId: param_id, condition: 'is_confirmed = true'});
      }
    });
  }
