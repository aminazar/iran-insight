...
class LCE {
  ...
  pushingLCENotification(tableName, joiner_id, lce_description, lce_description_fa, isConfirmed = false, other_joiner_id) {
  ...
  deleteLCE(type, user, lce_id) {
    return new Promise((resolve, reject) => {
      this.sql[`${type}_lce`].getLCEData({id: lce_id}).then(lce => {
        let exec, key;
        if (type === 'organization') {
          key = 'oid';
        ...
        exec.then(res => {
          if (res.isAdmin || res.isRep) { // is admin or rep of first
            this.sql[`${type}_lce`].delete(lce_id).then(() => {
              ...
              obj[`${type}_name`] = lce[0].former_name || lce[0].former_name_fa;
              obj[`${type}_id`] = lce[0].former_id;

              let obj1 = {};
              obj1[key] = lce[0].former_id;
              ...
              let obj2 = {}, obj3 = {};
              if (res.isAdmin) {
                obj2[key] = lce[0].former_id;
                obj3[key] = lce[0].seconder_id;

                NF.pushNotification(msg, obj2);
                if (lce[0].seconder_id)
            ...
            func.then(res => {
              if (res.isAdmin || res.isRep) { // is admin or rep of second joiner
                this.sql[`${type}_lce`].delete(lce_id).then(() => {
                  ...
                  obj2[`${type}_id`] = lce[0].seconder_id;
                  obj2[`${type}_name`] = lce[0].seconder_name || lce[0].seconder_name_fa;
                  ...
                  let obj3 = {}, obj4 = {};
                  obj3[key] = lce[0].former_id;
                  obj4[key] = lce[0].seconder_id;
  ...
  getLCE(type, user_pid, param_id) {
    ...
    return exec.then(res => {
      if (res.isAdmin || res.isRep) { // rep or admin is getting lce
        ...
        return this.sql[`${type}_lce`].getLCEList({joinerId: param_id, targetId: 'id1', condition: 'true = true'}); // show all LCE
      } else {
        return this.sql[`${type}_lce`].getLCEList({joinerId: param_id, targetId: 'id1', condition: 'is_confirmed = true'});
      }
    });
  }

  getRequestedLCE(type, user_pid, param_id) {
