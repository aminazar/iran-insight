...
class SearchSystem {
  ...
  callFunctions(el, queryObject, offset) {
    let query = queryObject.phrase ? queryObject.phrase.trim() : '';
    ...
    switch (el) {
      ...
      case 'consultancy': {
        ...
        break;
      case 'type': {
        return this.searchOnType(query, options.is_active, showAll, offset);
      }
      default:
        return null;
        break;
  ...
  searchOnType(phrase, isActive, showAll, offset) {
    return new Promise((resolve, reject) => {
      this.sqlTable.searchOnType({phrase: phrase, is_active: isActive, show_all: showAll, limit: limitation, offset: offset})
