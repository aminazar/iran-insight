        ...
        .then(res => {
          this.sql.membership.getBizOrgNameById({mid: mid})
            .then(result => {
              let msg = {
                from: user,
                about: Notification.get().getNotificationCategory().PersonJoinTo,
                aboutData: {
              ...
              Notification.get().pushNotification(msg, {pid: result[0].pid});
            });
    ...
    return new Promise((resolve, reject) => {
      ...
      this.canModifyMembership(user.pid, mid, aid)
        ...
        .then(res => {
          let msg = {
            from: user,
            about: Notification.get().getNotificationCategory().PersonRejectJoinRequest,
            aboutData: {
          ...
          Notification.get().pushNotification(msg, {pid: orgBizData.pid});
