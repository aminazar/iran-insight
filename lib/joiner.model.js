      ...
      curSql.person.isAdmin({pid: pid})
        .then(res => {
          if (res.length > 0) {          //admin is going to finish a representative's membership
            console.log('admin is going to finish a representative membership')
            return curSql.membership.get({mid: mid})
              .then((res) => {
                console.log(res.length === 1);
                console.log(res[0].end_time === null);
                console.log(res[0].end_time > new Date());
                if ((res.length === 1) && ( res[0].end_time === null || res[0].end_time > new Date()))
                  return curSql.membership.update({end_time: new Date()}, mid);
                else if (res[0].end_time < new Date()) {
                  console.log('this membership is finished');
                  return Promise.reject(error.membershipIsFinished);
                }
              })
          }
          else {
            console.log('rep or user is going to finish his/her membership');
