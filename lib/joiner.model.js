...
class Joiner {
  ...
  // deactiveExpiredMemberships(mid){
  //   return this.sql.membership.update({is_active : false}, mid)
  // }
  //
  ...
  //     .then(res => {
  //       console.log('/////',res);
  //       expired_members = res.filter( el => moment(el.membership_end_time).format('YYYY-MM-DD') < moment().format('YYYY-MM-DD'));
  //       console.log('------',expired_members);
  //       return Promise.all(expired_members.map(el => this.deactiveExpiredMemberships(el.mid)))
  //     })
  //     .then(() => {
  //       console.log('*****',expired_members);
  //       return this.sql.membership.getOrgBizMembers({bid, oid, offset, limit})
  ...
  upsertMembership(data, pid) {
    ...
    return new Promise((resolve, reject) => {
     return this.sql.person.isAdmin ({ pid : pid})
      .then(res => {
        if (res.length < 0)
          return Promise.reject(Err.adminOnly);
        else
          return this.sql.membership[query]({bid : bizOrgId})
      })
       .then(res => {
