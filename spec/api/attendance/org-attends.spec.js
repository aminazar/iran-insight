...
describe('PUT Organization Attendance API', () => {
  ...
  it('errors on unauthenticated put attempt', function (done) {
    rp({
      method: 'PUT',
      uri: lib.helpers.apiTestURL(`orgAttends/${eid}/${orgData.oid}`),
    })
      .then(() => {
  ...
  it('errors on non-existent event attendance', function (done) {
    rp({
      method: 'PUT',
      uri: lib.helpers.apiTestURL(`orgAttends/${eid + 10}/${orgData.oid}`),
      jar: aminJar
    })
      .then(() => {
